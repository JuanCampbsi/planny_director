    ...
    const renderColumn = (obj) => {
        return (React.createElement(S.Column, null,
            ...
            React.createElement(S.ModulosWrapper, null, obj.data.map((item) => {
                var _a, _b;
                return (React.createElement("a", { href: `https://globalvale.sharepoint.com/teams/PlanDirGestaoAtivos#/${isModalOpen ? 'Conteudo' : 'ContextoTema'}/` + item.Nome.trim(), onClick: () => handleSetFilterModule(item), role: "button" },
