...
export const Container = styled.div<Props>`
  ...
  align-items: center;
  width: 100%;
  position: ${({ sucess, error, isOpenEditForm }) => (sucess || error ? 'fixed' : 'absolute')};
  height: 100%;
  top: 0;
  left: 0;
  background: #0000001a 0% 0% no-repeat padding-box;
  z-index: 9998;
  padding-top: ${({ isPaddingFormEdit }) => (!!isPaddingFormEdit ? isPaddingFormEdit : '0')};
`;
